...
class CallFrame:
    ...
    def is_at_end(self):
        if self.ip<len(self.fn.opCodes):
            return False
        return True
    ...
    def __execute(self) -> None:
        while True:
            ...
            if instruction == OpCode.OP_RETURN:
                ...
                object = self.__chunk.constants[idx]
                self.__push(object)
                ...
                while instance.type == ObjectType.REF:
                    instance = self.__search_object_by_address(instance.pointer)
                if memberName.type == ObjectType.STR:
                    iter = instance.members.get(memberName.value, None)
                    if iter == None:
                        error("no member named:(" + memberName.__str__() +") in struct instance:" + instance.__str__())
                    self.__push(iter)

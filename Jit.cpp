...
JitFnDecl Jit::Compile(const CallFrame &frame, const std::string &fnName)
{
    ...
    while ((ip - frame.closure->function->chunk.opCodeList.data()) < frame.closure->function->chunk.opCodeList.size())
    {
        ...
        case OP_REF_INDEX_GLOBAL:
        {
            ...
            refObject = m_Builder->CreateCall(m_Module->getFunction(STR(AllocateIndexRefObject)), {globalVar, idxValue});
            Push(refObject);
            break;
        }
...
llvm::Value *Jit::CreateLlvmValue(const Value &value)
{
    ...
    else if (IS_STR_VALUE(value))
    {
        ...
        strObject = m_Builder->CreateCall(m_Module->getFunction(STR(AllocateStrObject)), {strObject});
        return strObject;
    }
    else
        return nullptr;

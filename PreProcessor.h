    ...
    TokenBlockTable FindBlockTable(std::vector<Token> tokens)
    {
        ...
        while (loc != -1)
        {
            if (tokens[loc + 1].type != TokenType::LPAREN)
                ASSERT("[line %lld]:Expect '(' after import keyword.", tokens[loc + 1].line);

            if (tokens[loc + 2].type != TokenType::STRING)
                ASSERT("[line %lld]:Expect file path after import stmt's '('.", tokens[loc + 2].line);

            if (tokens[loc + 3].type != TokenType::RPAREN)
                ASSERT("[line %lld]:Expect ')' after import stmt's file path.", tokens[loc + 3].line);

            if (tokens[loc + 4].type != TokenType::SEMICOLON)
                ASSERT("[line %lld]:Expect ';' at the end of import stmt.", tokens[loc + 4].line);

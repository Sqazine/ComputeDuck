...
std::vector<Token> PreProcessor::PreProcess(std::string_view src)
{
...
PreProcessor::TokenBlockTable PreProcessor::FindBlockTable(std::vector<Token> tokens)
{
	auto loc = SearchImportToken(tokens);
	if (loc == -1)
	{
...
int32_t PreProcessor::SearchImportToken(const std::vector<Token> &tokens)
{
